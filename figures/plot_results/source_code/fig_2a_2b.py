...
def roc_auc_scores(y_true, y_pred, features):
    ...
    for i, fea in enumerate(features):
        ...
        auc_scores[fea] = roc_auc_score(masked_y_true, masked_y_pred)
        assert round(auc(fpr[fea], tpr[fea]), 2) == round(roc_auc_score(masked_y_true, masked_y_pred), 2)
...
def generate_roc(y_true, y_pred, features, figsize=(2.3, 2.3), figname='Average_ROC_curves'):
    ...
    for feature in features:
        ...
        temp_df = pd.DataFrame({
            ...
            'Feature': '{0} (AUC = {1:0.2f})'.format(custom_labels[feature], auc_scores[feature])
        })
        all_data = pd.concat([all_data, temp_df])

    # Use seaborn to plot using 'Feature' as hue
    sns.lineplot(data=all_data, x='Specificity', y='Sensitivity', style='Feature', hue='Feature', palette='colorblind', linewidth=lw)
...
def precision_recall(y_true, y_pred, features):
    ...
    for i, fea in enumerate(features):
        ...
        # print(fea)
        # print(auc(recall[fea], precision[fea]), average_precision_score(masked_y_true, masked_y_pred))
        # assert round(auc(recall[fea], precision[fea]), 2) == round(average_precision_score(masked_y_true, masked_y_pred), 2)
...
def generate_pr(y_true, y_pred, features, figsize=(2.3, 2.3), figname='Average_PR_curves'):
    ...
    for feature in features:
        ...
        temp_df = pd.DataFrame({
            ...
            'Feature': '{0} (AP = {1:0.2f})'.format(custom_labels[feature], average_precision[feature])  # This column will be used for hue in the plot
        })
        all_data = pd.concat([all_data, temp_df])

    # Use seaborn to plot using 'Feature' as hue
    sns.lineplot(data=all_data, x='Recall', y='Precision', style='Feature', hue='Feature', palette='colorblind', linewidth=lw)
