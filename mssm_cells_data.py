...
def convert_label_to_onehot(df, num_classes):
    ...
    --------
    df_onehot : DataFrame
        Label列替换为Label_0, Label_1, ..., Label_n的数据框
    """
...
def process_ad_metadata_with_split(input_file, test_size=0.2, random_state=42):
    ...
    if 'AD_status' not in df.columns:
        print("❌ 错误：找不到AD_status列！")
        print(f"可用列名: {df.columns.tolist()}")
        return None

    AD_status_to_numeric = {
        'No': 0,
        'Yes': 1,
    }

    df['Label'] = df['AD_status'].map(AD_status_to_numeric)

    print(f"AD_status stage唯一值: {sorted(df['AD_status'].unique())}")
    print(f"\nLabel分布:")
    ...
    if df['Label'].isna().sum() > 0:
        print(f"\n⚠️  警告: 有{df['Label'].isna().sum()}行无法转换Label，将删除")
        df = df.dropna(subset=['Label'])
        print(f"✓ 删除后数据维度: {df.shape}")
    ...
    print(f"    列: {train_onehot.columns.tolist()}")
    ...
    print(f"  数值型特征: {len([c for c in train_encoded.columns if c not in categorical_cols and c != 'Label'])}")
    ...
    print(f"7. stage_2.toml - 模型配置文件 ⭐")
    if encoders:
        print(f"8. data/categorical_encoders.json - 分类特征编码映射")

    print("\n【下一步】")
    print("1. 检查 train_data_X.csv 确认列已正确删除")
    print("2. 运行训练：bash dev/train.sh")
...
def generate_toml_config(df, encoders, categorical_cols, num_label_classes, output_toml):
    ...
    print(f"  - 数值型特征: {len(numerical_features)}")
    print(f"  - 分类特征: {len(categorical_features)}")
    print(f"  - ID列: 未设置（避免按ID分组）")


if __name__ == "__main__":
    input_file = "mssm_cells_metadata.csv"

    print("\n" + "=" * 70)
    print(" AD细胞元数据处理脚本 (彻底清理版)")
    print("=" * 70)
    print(f"\n输入文件: {input_file}\n")
